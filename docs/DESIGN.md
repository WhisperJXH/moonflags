...
- `validate.mbt`：实现配置诊断。
- `presets.mbt`：提供常见业务场景的 Rule / Flag / Context 预设目录，便于项目初始化、示例演示和批量回归测试。
- `cmd/main`：提供最小可运行示例。
...
当前版本专注本地评估，不内置远程配置拉取、持久化、网络同步、加密随机数或 JSON/YAML 解析。核心逻辑保持稳定、可测试，并为外部配置加载层保留清晰的组合边界。

## 规模说明

为满足黑客松参考规模并提升可复用性，项目新增 `presets.mbt` 场景目录。该目录不是运行时必需的巨大配置文件，而是一组强类型模板：每个模板都能返回可验证的 `Rule`、`Flag` 和命中上下文，覆盖多国家、多套餐、多角色、多年龄门槛和不同比例 rollout。这样使用者可以直接复制或组合这些模板，也可以把它们作为本地评估引擎的回归样例。
